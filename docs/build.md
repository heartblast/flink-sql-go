...
기본 빌드도 `VERSION`의 안정 버전(현재 `0.1.3`)을 사용하고 `-dev`를 자동 추가하지 않는다. commit과 dirty 여부는 build-info에서 추적한다. 모든 version은 SemVer 2.0.0 검증을 통과해야 한다.
...
배포 산출물에는 `-flink-<version>` suffix가 붙는다. 예: `flink-sql-go-0.1.3-flink-1.20.4-source.zip`.
